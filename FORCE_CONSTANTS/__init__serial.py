...
class PhonoBase:
    ...
    * get_forces()
    ...
    def get_template(self, structure, supercell_matrix):
       ...
       # Important note:
       # When accessing per-atom data, please note that this data
       # is the per-processor local data and indexed accordingly.
       # These arrays can change sizes and order at every neighbor
       # list rebuild and atom sort event as atoms are migrating
       # between sub-domains.
       ...
       # Costly step for large systems; 
       # Thus done only once
       template = get_correct_arrangement(reference, structure, supercell_matrix)
    ...
    def get_force_constants(self, include_data_set=False):
        ...
        if self._force_constants is None:
            ...
            template = self.get_template(self._structure,\
                                   self._supercell_matrix)

            # Check forces for non displaced supercell
            forces_supercell = self.get_forces(phonon.get_supercell(), template)
            ...
            for i, cell in enumerate(cells_with_disp):
                ...
                forces = self.get_forces(cell, template)
...
class Phonolammps(PhonoBase):
    ...
    def get_forces_orig(self, cell_with_disp):
        """
    ...
    def get_forces(self, cell_with_disp, template):
        """
        Modified get_forces code for phonon calculations in 
        moir\'{e} materials. 
        ...
        @output
          numpy array matrix with forces of atoms [Natoms x 3]
        """
